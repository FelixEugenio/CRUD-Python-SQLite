...
# Criando a tabela tbl_pecas_revisao se não existir
with conexao:
    cur = conexao.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS tbl_pecas_revisao (
...
# Função para inserir dados na tabela tbl_pecas_revisao
def inserir_dados(i):
    with conexao:
        cur = conexao.cursor()
        query = "INSERT INTO tbl_pecas_revisao (id_reparacao,peca) VALUES (?,?)"
        cur.execute(query, i)

# Função para obter informações da tabela tbl_pecas_revisao
def mostrar_info():
    lista = []
    with conexao:
        cur = conexao.cursor()
        query = "SELECT * FROM tbl_pecas_revisao"
        cur.execute(query)
...
def inserir():
    id_reparacao = caixaTexto_id_reparacao.get()
    peca = caixa_peca.get()
    
    lista = [id_reparacao, peca]

    if not id_reparacao or not peca:
        messagebox.showerror('Erro', 'Todos os campos são obrigatórios')
    else:
        inserir_dados(lista)
        messagebox.showinfo('Sucesso', 'Os dados foram inseridos com sucesso')
        caixaTexto_id_reparacao.delete(0,'end')
        caixa_peca.delete(0,'end')
...
# Label de título
label_titulo = Label(frame_cima, text='Formulário de Revisao', anchor=NW, font=('Ivy 13 bold'), bg=co2, fg=co1, relief='flat')
label_titulo.place(x=10, y=20)

# Configurando Frame de baixo (campos de entrada)
label_id_viatura = Label(frame_baixo, text='Id_Reparacao *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
label_id_viatura.place(x=10, y=10)

caixaTexto_id_reparacao = Entry(frame_baixo, width=45, justify='left', relief='solid')
caixaTexto_id_reparacao.place(x=10, y=40)

label_peca = Label(frame_baixo, text='Peca *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
label_peca.place(x=10, y=70)

caixa_peca = Entry(frame_baixo, width=45, justify='left', relief='solid')
caixa_peca.place(x=10, y=100)
...
tabela_head = ['ID', 'ID_Reparacao', 'Peca']
